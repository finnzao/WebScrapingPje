...
def click_download_button_and_wait(typeDocument: DocumentoNome, process_number: str) -> bool:
    """
    1) Seleciona o tipo de documento;
    2) Verifica se o documento existe no processo
    3) Retorna True se o documento existe, False caso contrário
    """
    try:
        # Tenta selecionar o tipo de documento
        ...
        # Se conseguiu selecionar, o documento existe
        print(f"Documento '{typeDocument}' encontrado no processo {process_number}")
        return True
                
    except Exception as e:
        print(f"Erro ao verificar documento no processo {process_number}: {e}")
        return False
...
def downloadProcessOnTagSearch(typeDocument):
    """
    Verifica a presença de documentos nos processos e gera relatório
    """
    ...
    for index in range(1, total_processes + 1):
        ...
        try:
            print(f"\nVerificando processo {index} de {total_processes}")
            ...
            resultado_processo["Processo"] = process_number
            ...
            documento_existe = click_download_button_and_wait(typeDocument, process_number)
            resultado_processo["Documento"] = documento_existe
            ...
            print(f"Erro no processo {resultado_processo['Processo']}: {e}")
            resultado_processo["Documento"] = False
            ...
            resultados_verificacao.append(resultado_processo)
    ...
    df = pd.DataFrame(resultados_verificacao)
    
    # Cria nome do arquivo com timestamp
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    filename = f"./relatorios/verificacao_documentos_{timestamp}.xlsx"
    
    try:
        # Salva o arquivo Excel
        df.to_excel(filename, index=False, engine='openpyxl')
        print(f"\nPlanilha Excel salva em: {filename}")
    except Exception as e:
        print(f"Erro ao salvar Excel: {e}")
        # Tenta salvar como CSV como alternativa
        csv_filename = f"./relatorios/verificacao_documentos_{timestamp}.csv"
        df.to_csv(csv_filename, index=False, encoding='utf-8-sig')
        print(f"Planilha CSV salva em: {csv_filename}")
    
    # Salva também em JSON para garantir
    json_filename = f".logs/verificacao_documentos_{timestamp}.json"
    df.to_json(json_filename, orient='records', force_ascii=False, indent=4)
    print(f"Relatório JSON salvo em: {json_filename}")
    
    # Exibe resumo
    total_com_documento = df['Documento'].sum()
    total_sem_documento = len(df) - total_com_documento
    
    print("\n========== RESUMO DA VERIFICAÇÃO ==========")
    print(f"Total de processos verificados: {len(df)}")
    print(f"Processos com o documento '{typeDocument}': {total_com_documento}")
    print(f"Processos sem o documento: {total_sem_documento}")
    print("==========================================")
    
    # Mostra primeiros registros para confirmação
...
def main():
    ...
    try:
        ...
        # Executa a verificação de documentos
        downloadProcessOnTagSearch(typeDocument="Sentença")
