...
# 1) Importamos a classe do outro arquivo:
...
def nav_tag():
    """
    Apenas clica no menu lateral para abrir a seção de etiquetas.
    """
...
def downloadProcessOnTagSearch(typeDocument):
    ...
    Se o tipo de documento não existir para algum processo,
    ...
    for index in range(1, total_processes + 1):
        try:
            ...
            if len(process_number) >= 17:
                process_number = (
                    f"{process_number[:7]}-{process_number[7:9]}."
                    f"{process_number[9:13]}.{process_number[13]}."
                    f"{process_number[14:16]}.{process_number[16:]}"
                )
            else:
            ...
            # Fecha a janela do processo atual e retorna para a original
            driver.close()
...
def iniciar_automacao():
    ...
    - Instanciar a classe do pje_automation
    - Fazer login, escolher perfil e expor driver/wait como globais
    """
    load_dotenv()
...
def main():
    ...
    try:
        ...
        processos_encontrados = downloadProcessOnTagSearch("Ofício")
        download_requested_processes(processos_encontrados, etiqueta="Sentenciado")
        time.sleep(5)
    finally:
        automator.close()
